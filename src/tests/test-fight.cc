...
TEST_F(GameTest, SeaFight1)
{
    ...
    INFO("get_player");
    EXPECT_EQ(-1, sea_cell->get_player())
        << "Cell should belong to nobody before first fight";
    ...
    EXPECT_NE(-1, count_boats(pos, 0))
        << "The wrong boats were deleted from boats std::map<int, bateau>";
    EXPECT_EQ(1, count_boats(pos, 0)) << "Winner's boat must be remaining";

    EXPECT_NE(-1, count_boats(pos, 1))
        << "The wrong boats were deleted from boats std::map<int, bateau>";
    EXPECT_EQ(0, count_boats(pos, 1)) << "Loser's boat must be destroyed";

    EXPECT_EQ(-1, sea_cell->get_player())
        << "Cell should still belong to nobody after the fight";
}
...
TEST_F(GameTest, SeaFight2)
{
    ...
    EXPECT_EQ(-1, sea_cell->get_player())
        << "Sea cell should still belong to nobody after the fight";
}
...
TEST_F(GameTest, IslandFight1)
{
    ...
    EXPECT_EQ(1, island_cell->get_player())
        << "Before the fight, island belongs to 1";
    gamestate_->resolve_fight(pos, 0);
    EXPECT_EQ(1, island_cell->get_player())
        << "After the fight, still island belongs to 1";
}
...
TEST_F(GameTest, IslandFight2)
{
    ...
    gamestate_->resolve_fight(pos, 0);
    EXPECT_EQ(1, island_cell->get_player())
        << "After the fight, island still belongs to 1";
}
...
TEST_F(GameTest, IslandFight3)
{
    ...
    EXPECT_EQ(0, island_cell->get_player())
        << "Before the fight, island belongs to 0";
    gamestate_->resolve_fight(pos, 1);
    EXPECT_EQ(1, island_cell->get_player())
        << "After the fight, island belongs to 1";
}
...
TEST_F(GameTest, IslandFight4)
{
    ...
    gamestate_->resolve_fight(pos, 1);
    EXPECT_EQ(0, island_cell->get_player())
        << "After the fight, island still belongs to 0";
}
